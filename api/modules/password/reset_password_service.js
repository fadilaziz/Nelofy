...
export const request_change_password = async (data) => {
  try {
    ...
    //Ambil data expiry untuk di hitung
    const expiryData = await sql`
        SELECT reset_token_expiry
        FROM users
        WHERE user_email = ${data.email}`;

    // Hitung 3 menit jika token kedaluwarsa maka hapus toke yang ada di databse
...
export const update_password = async (data) => {
  try {
    ...
    await sql`
        UPDATE users
        SET user_pass = ${hashPassword}, reset_token = NULL, reset_token_expiry = NULL
