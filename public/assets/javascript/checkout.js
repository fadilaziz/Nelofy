(function initNavbarChip() {
      ...
      .then(data => {
        if (data.code === 200) {
          ...
          <div class="customer-info-box">
            <div class="customer-avatar">${data.data.full_name.split(' ').map(w => w[0]).join('').toUpperCase().slice(0,2)}</div>
    ...
    function calculateTotals(newSubtotal) {
      ...
      document.getElementById('grand-total-val').textContent = `Rp ${finalGrandTotal.toLocaleString('id-ID')}`;
    }
    ...
    function checkoutToPayment() {
      ...
      const payload = {
        ...
        // paymentMethodName: selectedOpt.name,
        // paymentNumber: selectedOpt.number,
        // isQris: selectedOpt.isQris,
        // timestamp: now.getTime(),
        // expiresAt: expiry.getTime() // Expires in 24 hours
