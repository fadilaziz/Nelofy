(function initNavbarChip() {
    ...
    function updateStatusBanner(status) {
      ...
      if (status === 'pending') {
        ...
        banner.querySelector('.status-banner-icon').innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>`;
        ...
        banner.querySelector('.status-banner-icon').innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>`;
    ...
    function renderPayment(paymentData) {
      ...
      // Total
      document.getElementById('val-total-tagihan').textContent = formatRupiah(d.total_tagihan || d.total_amount);
      ...
      // Detail grid
      document.getElementById('val-tanggal').textContent = formatDateTime(d.tanggal_pesan || new Date());
      ...
      document.getElementById('val-invoice-status').textContent = 
        ...
        (status === 'paid' || status === 'success') ? 'Lunas' : status.toUpperCase();
      document.getElementById('val-jatuh-tempo').textContent = formatDateTime(d.jatuh_tempo || d.expired_at);
      ...
      // Summary
      document.getElementById('val-subtotal').textContent = formatRupiah(d.total_order || d.total_amount);
    ...
    async function loadPaymentData() {
      ...
      const rawOrderId = localStorage.getItem('bp_pending_order_id');
      if (!rawOrderId) {
        loadingEl.style.display = 'none';
        errorEl.style.display = 'block';
        document.getElementById('error-message').textContent = 'Tidak ada order ID yang ditemukan. Silakan lakukan checkout terlebih dahulu.';
        return;
      }

      console.log('Order ID:', rawOrderId);
